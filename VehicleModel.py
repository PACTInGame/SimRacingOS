...
class VehicleModel:
    ...
    def update_outsim(self, packet):
        ...
        self.gear = packet.Gear
        self.engine_angular_velocity = packet.EngineAngVel
        self.max_torque_at_velocity = packet.MaxTorqueAtVel
        self.current_lap_distance = packet.CurrentLapDist
        self.indexed_distance = packet.IndexedDistance
        self.steer_torque = packet.SteerTorque
        ...
        for i, wheel in enumerate(packet.Wheels):
            ...
            self.tire_data[i]["lean_rel_to_road"] = wheel["LeanRelToRoad"]
            self.tire_data[i]["air_temperature"] = wheel["AirTemp"]
            self.tire_data[i]["slip_fraction"] = wheel["SlipFraction"]
            self.tire_data[i]["slip_ratio"] = wheel["SlipRatio"]
            self.tire_data[i]["tan_slip_angle"] = wheel["TanSlipAngle"]
